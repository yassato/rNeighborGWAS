#' Linear mixed models for testing self and neighbor effects
#'
...
#' @param g_nei An output of \code{nei_coval()} object, namely a individual x marker matrix including neighbor genotypic identity.
...
#' }
#' @details This function gives detailed results but requires more computational time. For an easier use, please run \code{neiGWAS()} alternatively.
#' @author Yasuhiro Sato (\email{sato.yasuhiro.36c@kyoto-u.jp})
#' @import Matrix gaston parallel
#' @export
nei_lmm = function(geno, g_nei, pheno, addcovar=NULL, response="quantitative", n_core=1L) {
  ...
  test_marker_i = function(i) {
    if(response=="quantitative") {
      LL_self0 <- gaston::lmm.diago.profile.likelihood(tau=res00$tau, s2=res00$sigma2, Y=pheno, X=cbind(addcovar, stats::model.matrix(~geno[,i])), eigenK=eiKs)[1,1]
      p_self <- stats::pchisq(-2*(LL00-LL_self0), 1, lower.tail=FALSE)

      LL_self1 <- gaston::lmm.diago.profile.likelihood(tau=res01$tau, s2=res01$sigma2, Y=pheno, X=cbind(addcovar, stats::model.matrix(~geno[,i])), eigenK=eiK)[1,1]
      LL_nei <- gaston::lmm.diago.profile.likelihood(tau=res01$tau, s2=res01$sigma2, Y=pheno, X=cbind(addcovar, stats::model.matrix(~geno[,i]+g_nei[,i])), eigenK=eiK)[1,1]
      p_nei <- stats::pchisq(-2*(LL_self1-LL_nei), 1, lower.tail=FALSE)

      lmm_nei <- gaston::lmm.diago(Y=pheno, cbind(addcovar,stats::model.matrix(~geno[,i]+g_nei[,i])), eigenK=eiK, verbose=FALSE)
      beta <- lmm_nei$BLUP_beta[-c(1:(ncol(addcovar)+1))]

      resList <- c(beta,p_self,p_nei)
      return(resList)
    } else if(response=="binary") {
      res <- gaston::logistic.mm.aireml(Y=pheno,X=cbind(addcovar,stats::model.matrix(~geno[,i]+g_nei[,i])),K=list(K_self,K_nei),verbose=FALSE)
      beta <- res$BLUP_beta[-c(1:(ncol(addcovar)+1))]
  ...
  results <- parallel::mcmapply(test_marker_i,1:q, mc.cores=getOption("mc.cores", n_core))
  results <- t(results)
