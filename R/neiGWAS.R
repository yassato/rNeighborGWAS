...
#' @export
neiGWAS = function(geno, pheno, gmap, smap, scale, addcovar=NULL, grouping, response="quantitative", model="lmm", n_core=1L) {
  ...
  test_marker_i = function(i) {
    if(model=="lmm") {
      if(response=="quantitative") {
        LL_self <- gaston::lmm.diago.profile.likelihood(tau=res0$tau, s2=res0$sigma2, Y=pheno, X=cbind(addcovar,stats::model.matrix(~geno[,i])), eigenK=eiK)[1,1]
        LL_nei <- gaston::lmm.diago.profile.likelihood(tau=res0$tau, s2=res0$sigma2, Y=pheno, X=cbind(addcovar,stats::model.matrix(~geno[,i]+g_nei[,i])), eigenK=eiK)[1,1]
        ...
        p_nei <- stats::pchisq(z_val^2,1,lower.tail=FALSE)[length(z_val)]
        return(p_nei)
      }
    } else {
      if(response=="quantitative") {
        X <- cbind(addcovar,stats::model.matrix(~geno[,i]))
        LL_self_lm <- stats::logLik(stats::lm(pheno~X-1))
        X <- cbind(addcovar,stats::model.matrix(~geno[,i]+g_nei[,i]))
