...
test_that(
  ...
  code = {
    ...
    expect_true(min(-log10(gwas_lmm[,3]),na.rm=TRUE)>=0)

    expect_true(min(-log10(gwas_glmm[,3]),na.rm=TRUE)>=0)

    expect_true(min(-log10(gwas_lm[,3]),na.rm=TRUE)>=0)

    expect_true(min(-log10(gwas_glm[,3]),na.rm=TRUE)>=0)
  }
)
